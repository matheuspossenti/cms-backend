...
export class CreateAccountUseCase {
  ...
  async execute({
    ...
    await validate(createAccountYupSchema, { email, name })
...
const createAccountYupSchema = object({
  name: string().min(3, 'Nome deve conter ao menos 3 caracteres').required('Nome é obrigatório'),
  email: string().email('Deve ser um email válido').required('E-mail é obrigatório'),
  role: string().oneOf(['editor', 'redator'], 'Função deve ser editor ou redator').required('Função é obrigatória'),
})
